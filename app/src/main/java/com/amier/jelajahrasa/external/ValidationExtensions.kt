...
fun String.validateEmail(context: Context): ValidationModel {
    return if (this.trim().isEmpty()) {
        ValidationModel(false, context.getString(R.string.all_email_empty))
    } else if (!Patterns.EMAIL_ADDRESS.matcher(this).matches()) {
        ValidationModel(false, context.getString(R.string.all_email_format_wrong))
    } else {
...
fun String.validatePassword(context: Context): ValidationModel {
    ...
    return if (this.isEmpty()) {
        ValidationModel(false, context.getString(R.string.all_pasword_empty))
    } else if (this.length < 5) {
        ValidationModel(false, context.getString(R.string.all_password_min))
    } else if (!pattern.containsMatchIn(this) || !pattern2.containsMatchIn(this)) {
        ValidationModel(false, context.getString(R.string.all_password_combination))
    } else {
        ValidationModel(true, "")
    }
}
